...
u (d-dim, dal compressore)
        ↓
fit_scaler (SOLO su train) + apply_scaler → [0, π] + pad_features
        ↓
...
|:---|:---|:---|
| `ENCODING_SCALE_MAX` | `π` — decisione finale | Con `RY(θ)`, `⟨Z⟩=cos(θ)` non è iniettivo su `[0,2π]`. Crollo empirico di 25-30 punti di macro-F1 confermato con `2π`. |
